 ...
 * 
 * @param logoPathOverride - Optional path to logo file (for extension use with AssetResolver)
 * @returns Promise that resolves when PDF is generated successfully
 * @throws Error with descriptive message if any step fails
 */
export async function exportPdf(logoPathOverride?: string): Promise<void> {
  try {
    ...
    const logoPath = logoPathOverride || 'templates/logo.png'; // Use override if provided, otherwise default
