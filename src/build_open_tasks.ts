...
async function executeTask(
  ...
  // First, try scripts directory relative to the extension's location
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("so-workspace.exportPdfNpm", async () => {
      ...
      try {
        ...
        log(`[SO] Running PDF export for workspace: ${workspaceFolder.uri.fsPath}`);

        // Import and call the PDF export function directly
        const { exportPdf } = require("./pdf-export/index");
        await exportPdf();
        
        vscode.window.showInformationMessage("PDF export completed successfully");
      } catch (err: any) {
