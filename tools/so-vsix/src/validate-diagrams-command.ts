...
import { StructurizrValidator, ValidationResult } from "./structurizr-validator";
...
async function executeValidateDiagrams(): Promise<void> {
  try {
    ...
    // Get Structurizr server URL from settings
    const serverUrl = getStructurizrServerUrl();
    ...
    // Validate files
    await validateFiles(dslFiles, serverUrl, outputChannel);
...
async function validateFiles(
  ...
  // Validate all files
  const results = await validator.validateAll(dslFiles, serverUrl);
 ...
 * Get Structurizr server URL from settings
 * 
 * Reads the server URL from VSCode settings.
 * Falls back to default localhost URL if not configured.
 * 
 * @returns Structurizr server URL
 * 
 * Requirements:
 * - 13.5: Read server URL from configuration with default value
 * - 13.7: Support both Structurizr Cloud and self-hosted Lite
 */
function getStructurizrServerUrl(): string {
  const config = vscode.workspace.getConfiguration("so-workspace");
  const serverUrl = config.get<string>("diagrams.structurizrServerUrl");
  
  // Default to Structurizr Lite on localhost (Requirement 13.5)
